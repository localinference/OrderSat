...
DEFAULT_DISTRIBUTIONS_ROOT = pathlib.Path("models/distributions")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Export a trained seq2seq checkpoint to an ONNX/browser distribution "
            "under models/distributions/{language}."
        )
    )
    parser.add_argument(
        "language_positional",
        nargs="?",
        help="Language directory under models/best_models, models/tokenizers, and models/distributions.",
    )
    parser.add_argument(
        "--language",
        dest="language_flag",
    ...
    parser.add_argument(
        "--distributions-root",
        default=str(DEFAULT_DISTRIBUTIONS_ROOT),
        help="Distribution root directory (default: models/distributions)",
    )
    parser.add_argument(
        "--checkpoint-name",
...
def resolve_paths(args: argparse.Namespace) -> ExportPaths:
    ...
    distributions_root = pathlib.Path(args.distributions_root).resolve()

    checkpoint_dir = best_models_root / args.language
    tokenizer_dir = tokenizers_root / args.language
    distribution_dir = distributions_root / args.language
    ...
    return ExportPaths(
        ...
        onnx_model_path=distribution_dir / args.onnx_model_name,
        config_path=distribution_dir / args.config_name,
        exported_metrics_path=distribution_dir / args.metrics_name,
        exported_tokenizer_model_path=distribution_dir / args.tokenizer_model_name,
        exported_tokenizer_vocab_path=distribution_dir / args.tokenizer_vocab_name,
    )
...
def copy_artifacts(paths: ExportPaths) -> None:
    paths.distribution_dir.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    print(f"Distribution dir: {paths.distribution_dir}")
